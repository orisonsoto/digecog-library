...
export function PanelDerecho() {
  ...
  if (!abierto) {
    return (
      // Solo en escritorio: en móvil el panel se abre desde el botón "Mía AI" del topbar,
      // así no se superpone una pestaña flotante sobre el contenido.
      <button
        onClick={() => setAbierto(true)}
        aria-label="Abrir panel de alertas y Mía AI"
        className="hidden lg:flex fixed right-3 top-1/2 -translate-y-1/2 z-30 rounded-full bg-white shadow-lg border border-[var(--border-subtle)] p-2 text-[var(--text-muted)] hover:text-[var(--color-brand-600)]"
      >
        ‹
      </button>
    );
  ...
  return (
    <aside
      className={
        // Móvil: hoja superpuesta a pantalla completa. Escritorio (lg+): columna fija del layout.
        'fixed inset-0 z-50 flex flex-col bg-white ' +
        'lg:static lg:inset-auto lg:z-auto lg:w-[320px] lg:shrink-0 lg:h-full lg:border-l lg:border-[var(--border-subtle)]'
      }
    >
      <div className="flex items-center justify-between px-3 h-14 border-b border-[var(--border-subtle)] shrink-0">
        ...
        <button
          ...
          className="h-10 w-10 -mr-1 flex items-center justify-center rounded-lg text-[var(--text-muted)] hover:bg-slate-100 hover:text-[var(--text-primary)]"
        >
          <X size={18} />
        </button>
      </div>
